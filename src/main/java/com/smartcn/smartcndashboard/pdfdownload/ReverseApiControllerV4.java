...
public class ReverseApiControllerV4 {

	@Autowired
	ReverseApiService ReverseApiService;

	public ReverseApiControllerV4(ReverseApiService reverseApiService) {
		super();
		ReverseApiService = reverseApiService;
	}
	...
	public ResponseEntity<StatusMessage> fetchData(@PathVariable("clientcode") String clientcode,
		...
		try {
			...
			if (segment.equals("all")) {
				...
				String[] allSegments = { "stt", "pnl", "dpholding", "commcn", "commodity", "bill", "dpledger", "ros",
						"agts", "dpholdingyearly", "qsledger", "dptrade", "qsretention" };
				...
				for (String segments : allSegments) {
					...
					if ((startDate.before(endDate) && endDate.before(new Date())) || (startDate.equals(endDate))) {
						...
						for (Iterator iterator = totalDates.iterator(); iterator.hasNext();) {
							...
							key = folderName + currentYear + "/" + currentDateMonthStr + "/" + currentDayStr + "/"
									+ clientcode + "/";
